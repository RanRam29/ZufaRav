...
def create_event_logic(event, conn):
    ...
    try:
        ...
        conn.rollback()
        logger.error(f"❌ שגיאה ביצירת אירוע '{event.title}': {str(e)}")
...
def update_people_count_logic(event_id, new_count, conn):
    logger.debug(f"🔧 עדכון כמות משתתפים באירוע {event_id} ל-{new_count}")
    try:
        with conn.cursor() as cursor:
            cursor.execute("""
            ...
            """, (new_count, event_id))
            if cursor.rowcount == 0:
                logger.warning(f"⚠️ ניסיון לעדכן אירוע שלא נמצא: ID {event_id}")
                raise HTTPException(status_code=404, detail="אירוע לא נמצא")
        conn.commit()
        logger.info(f"✅ עודכנו {new_count} משתתפים לאירוע {event_id}")
        return {"msg": f"עודכנו {new_count} משתתפים לאירוע {event_id}"}
    except Exception as e:
        conn.rollback()
        logger.error(f"❌ שגיאה בעדכון משתתפים באירוע {event_id}: {str(e)}")
