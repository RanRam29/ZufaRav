...
async def websocket_endpoint(websocket: WebSocket):
    ...
    logger.info("info", f"🔗 לקוח התחבר לוובסוקט (סה״כ {len(connected_clients)} מחוברים)")
    ...
    try:
        ...
        logger.warning("warning", f"🔌 לקוח התנתק מוובסוקט (סה״כ {len(connected_clients)} מחוברים)")

async def broadcast_new_event(event_data):
    logger.debug("debug", f"📡 שידור אירוע חדש לכל הלקוחות: {event_data.get('title', 'ללא כותרת')}")
